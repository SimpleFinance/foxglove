...
(defn message!
  ...
  [url services sender options message & {:keys [exceptions? local-testing]}]
  (when-not local-testing
    (let [payload {:message  message
      ...
      (log/infof
        "url=%s message=%s sender=%s services=%s" full-url message sender services)
